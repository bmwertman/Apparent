...
// .constant('gapiClientId', '125323192420-c2nscbgoel9d0m7jv400dcfhfmtomac2.apps.googleusercontent.com')
  .run([
    ...
    'userService',
    '$http',
    function ($ionicPlatform, $cordovaPushV5, $rootScope, Auth, editableThemes, editableOptions, $localstorage, $state, $compile, $cordovaDevice, $ionicPopup, $timeout, userService, $http) {
        ...
        $rootScope.$on('$cordovaPushV5:notificationReceived', function (event, notification) {
          ...
          if (data.sender_imgUrl !== user.pic && data.foreground) { // Make sure the user isn't getting notified about their own message
            ...
            if ($rootScope.toParams === null || $rootScope.toParams.roomId !== data.roomId) {
              ...
              var divTag = angular.element(document.createElement('div'))
              var chatterIcon = angular.element(document.createElement('div'))
              ...
              if (!queue) { // This is the first queued chatter
                ...
                var iconBackground = angular.element(document.createElement('div'))
                var queue = angular.element(document.createElement('div'))
                queue.attr({
                  id: 'queue',
                  dragula: '"chatter-bag"',
              ...
              if ($rootScope.queuedChatters.length > 0) {
                for (var i = $rootScope.queuedChatters.length - 1; i >= 0; i--) {
                  var value = $rootScope.queuedChatters[i]
